...
#makes a folder with the appropiate separation of images based on the labels
#THe images are separated to folders so it is adequate for the load_dataset function from dataset
def makeFolder(df,folder_path,target_column_name:str):
    
    destination = f'resources/externaldata/{target_column_name}'
   
   #creates both the positive and negative folders
    destination_positive = f'resources/externaldata/{target_column_name}/{df.columns[1]}'
    destination_negative = f'resources/externaldata/{target_column_name}/No {df.columns[1]}'
    
     # Create the new directory
    os.makedirs(destination_positive, exist_ok=True)
    # Create the new directory
    os.makedirs(destination_negative, exist_ok=True) 
    #Adds the appropriate image based on the label to the appropriate folder
...
#given a folder with the models, it returns a list with the models
